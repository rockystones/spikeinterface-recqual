...
**A false alarm was raised and retracted in the process.** `SN 1025-004377` — Rocky implant-2 anterior — places `elec18` at top-left and `elec8` at bottom-right, leaving `(8,9)` and `(9,8)` vacant instead of the usual `(0,9)` and `(9,0)`. S07's validator assumed the symmetric layout, called that a defect, and "repaired" it onto cells that hold no electrode.
