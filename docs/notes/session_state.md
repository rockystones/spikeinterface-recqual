...
**Potentiostat EIS ordering — the next thing to act on.** The Blackrock 1 kHz impedance is settled: indexed by `channel_id`, proved at 1,248/1,248 positions. The experimenter's own full-spectrum EIS/CV data (13 arrays, `Characterization/10by10/`) is *not* — 32 sweeps per bank file, but sweep-to-pin order unverified. The best lead is the partial-saline-submersion protocol that appears in **LB-0514 revision 4.00 only** and was removed in 5.00; it imposes a known spatial pattern, which is the one thing that identifies an ordering without assuming it. See [`impedance_sources.md`](impedance_sources.md). Publish no per-electrode EIS conclusion until then.
